...
import { ChevronDown, Trash2, Building2, DollarSign, BadgeCheck, AlertCircle, AlertTriangle, Users, Plus } from "lucide-react";
...
import { STATE_DATA, isEntityComplete, runEngagementCalculation } from "@/utils/calculatorEngine";
...
export function EntityCard({ entity, index }: Props) {
  ...
  const allEntities = useCalculatorStore((s) => s.entities);

  const engagementResult = useMemo(() => {
    const completeEntities = allEntities.filter(isEntityComplete);
            ...
            <div className="grid gap-6 p-5 md:grid-cols-2 [&>*:last-child]:md:col-span-2">
              ...
              <div>
                ...
                <div className="grid gap-4">
                  ...
                  {/* Eligibility alert — mutually exclusive with minimum threshold warning */}
                  {engagementResult && engagementResult.tier === "Out of Range" ? (
                    <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive">
                      <span className="inline-flex items-center gap-2 font-medium">
                        <AlertTriangle className="h-4 w-4" /> Federal Credit Estimate below $6,000 minimum. Please review inputs.
                      </span>
                    </div>
                  ) : complete && stateEligible ? (
              ...
              {/* Notes — full width, before Ownership Breakdown */}
