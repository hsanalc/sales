// Client Dashboard — KPI-driven filtering, search, and sortable table.
...
import { YearChips } from "@/components/common/MultiYearSelect";
...
function DashboardTable(p: TableProps) {
  return (
        ...
        <tbody>
          ...
          {p.rows.map((l) => (
            <motion.tr
              ...
              className="cursor-pointer border-b border-border transition-colors hover:bg-accent/40 last:border-0"
            >
...
function EntityCell({ names, count }: { names: string[]; count: number }) {
  ...
  return (
    <div className="space-y-0.5">
      {visible.map((name) => (
        <p key={name} className="text-sm leading-snug text-navy">
          {name}
        </p>
      ))}
      {overflow > 0 && !expanded && (
