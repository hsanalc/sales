...
import { MultiYearSelect } from "@/components/common/MultiYearSelect";
...
import { runEngagementCalculation, isEntityComplete } from "@/utils/calculatorEngine";
...
export function CalculatorPage() {
  ...
  const totals = useMemo(() => {
    ...
    return { federalCreditEstimate, federal, stateTotal, finalBill };
  }, [result]);
  ...
  // Overview reflects ONLY the active year for strict year-based data consistency.
  const yearsLabel =
    client.taxYears.length === 0 ? "No years selected" : String(activeYear);
  ...
  const handleDownload = async () => {
    const years = client.taxYears.length > 0 ? client.taxYears : [activeYear];
    setDownloading(true);
    const originalYear = activeYear;
    try {
      ...
      for (const year of years) {
        // Switch to this year and wait for React + layout to commit.
        setActiveYear(year);
        ...
        pdf.text(`Billing Overview Summary — ${year}`, margin, margin + 4);
        const headerOffset = margin + 24;
        ...
        if (imgHeight <= availableHeight) {
          ...
          const firstSlicePx = availableHeight * pxPerPt;
          const restSlicePx = (pageHeight - margin * 2) * pxPerPt;
          let renderedPx = 0;
          ...
          while (renderedPx < canvas.height) {
            const sliceMax = pageIdx === 0 ? firstSlicePx : restSlicePx;
            const sliceHeight = Math.min(sliceMax, canvas.height - renderedPx);
            ...
            const yOffset = pageIdx === 0 ? headerOffset : margin;
      ...
      // Restore the year the user was viewing.
      setActiveYear(originalYear);
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
      ...
      {client.taxYears.length > 0 && (
        <div className="sticky top-2 z-20 flex flex-wrap items-center gap-2 rounded-xl border border-border bg-card/95 p-2 shadow-card backdrop-blur">
          <span className="px-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
            Year
          </span>
          {client.taxYears.map((y) => {
            const isActive = y === activeYear;
            const slice = byYear[y];
            const hasData = !!slice && slice.entities.length > 0;
            return (
              <button
                key={y}
                type="button"
              ...
              >
                {y}
                {hasData && !isActive && (
                  <span className="ml-2 inline-block h-1.5 w-1.5 rounded-full bg-green" />
                )}
              </button>
            );
          })}
      ...
      <Section icon={<Layers className="h-4 w-4 text-violet" />} title={`Generate Entities${client.taxYears.length > 0 ? ` (${activeYear})` : ""}`}>
      ...
      <Section
        ...
        title={`Entity Details & Calculations${client.taxYears.length > 0 ? ` (${activeYear})` : ""}`}
        note={null}
        action={
          entities.length > 0 && (
      ...
      <Section id="billing-overview-summary" title={`Billing Overview Summary${client.taxYears.length > 0 ? ` (${activeYear})` : ""}`} icon={<DollarSign className="h-4 w-4 text-green" />}>
